//
// Created by Joohan on 12/3/2021.
//
...
// and now use the IOCTL to get the time the packet was stamped by the kernel.
    if ( 0 > ioctl(sock, SIOCGSTAMP, &tv_ioctl)) {
        assert(false && "ioctl failed!");
    }
    printf ("gettimeofday: %ld\t ioctl: %ld\t delta: %d.%d\n",
            tv_tod.tv_sec * 1000000 + tv_tod.tv_usec,
            tv_ioctl.tv_sec * 1000000 + tv_ioctl.tv_usec,
